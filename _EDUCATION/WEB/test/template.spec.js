...
describe("{{expression}}", function () {
  it("should bind expressions with the context.", function () {
    ...
    expect(template(source, context)).toBe(
  ...
  it("should access the value with brackets if value is an object or array.", function () {
    expect(template("<p>{{ arr[2] }}</p>", { arr: [0, 1, 2] })).toBe(
      "<p>2</p>"
    );
    expect(template('<p>{{obj["key"]}}</p>', { obj: { key: "value" } })).toBe(
      "<p>value</p>"
    );
    expect(
    ...
    expect(
      template("{{each nums}}{{nums[@index]}}{{/each}}", { nums: [1, 2, 3] })
    ).toBe("123");
  });
  ...
  it("should bind with string if value is string with quotes.", function () {
    ...
    expect(template('<p>{{ sayHello "CodeSnippet" }}</p>', context)).toBe(
      "<p>Hello, CodeSnippet</p>"
    );
    expect(template("<p>{{sayHello 'world'}}</p>", context)).toBe(
      "<p>Hello, world</p>"
    );
  });
});
...
describe("{{helper arg1 arg2}}", function () {
  it("should execute a custom helper function with arguments.", function () {
    var source =
      '<div class="{{getClassNamesByStatus disabled prefix}}"></div><div class="{{getClassNamesByStatus enabled}}"></div>';
    var context = {
      disabled: true,
      enabled: false,
      prefix: "item-",
    ...
    expect(template(source, context)).toBe(
      '<div class="item-disabled"></div><div class=""></div>'
    );
...
describe("{{if ...}} {{elseif ...}} {{else}} ... {{/if}}", function () {
  it("should use if expression as a helper function.", function () {
    ...
    expect(template(source, { content: "Hello, world!" })).toBe(
  ...
  it("should use elseif, else expression.", function () {
    ...
    source =
      "<p>{{if equals n 1}}{{n}} is one{{ elseif equals n 2 }}{{n}} is two{{else}}{{n}} is other{{/if}}</p>";
    context.n = 1;
  ...
  it("should use each expression in the if expression.", function () {
    var source =
      "<p>{{if isNumber arr[0]}}Number: {{each arr}}{{@this}}{{/each}}{{elseif isString arr[0]}}String: {{each arr}}{{@this}}{{/each}}{{else}}Nothing{{/if}}</p>";
    var context = {
      isNumber: function (n) {
  ...
  it("should use if expressions in the if expression.", function () {
    ...
    source =
      "It is {{if smaller n 5}}smaller than 5 and {{if equals n 2 }}same as 2 {{else}}not same as 2 and {{if equals n 3}}same as 3 {{/if}}{{/if}}{{else}}bigger than 5 and {{if equals n 5}}same as 5 {{else}}not same as 5 {{/if}}{{/if}}";
    context = {
    ...
    expect(template(source, context)).toBe(
      "It is smaller than 5 and not same as 2 and same as 3 "
    );
  });
});
...
describe("{{each ...}} @this @index @key {{/each}}", function () {
  it("should use each expression as a helper function.", function () {
    ...
    source =
      "{{each getPositiveNumbersSmallerThanFive 3}}<p>{{@this}}</p>{{/each}}";
    expect(
      template(source, {
  ...
  it("should use if expression in the each expression.", function () {
    var source =
      "{{each numbers}}<p>{{@this}}{{if equals @this 2}} is even{{elseif equals @this 0}} is zero{{else}} is odd{{/if}}</p>{{/each}}";
    expect(
      template(source, {
        numbers: [1, 2, 0],
        equals: function (a, b) {
